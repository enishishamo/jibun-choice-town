...
export default function SafetyPlanGame({ onComplete }: Q1GameProps) {
  ...
  // 2026-09-13 UX audit fix: the roles step used to show only each adult's
  // icon+name, with no way to see which band they were assigned to in the
  // previous step -- but computeIssues requires matching 花組/月組's OWN
  // adult to a specific role, so the player had to perfectly memorize a
  // 5-item mapping made moments earlier with zero on-screen reference. This
  // doesn't hand over the answer (which role each band needs is still not
  // stated), it just makes the band-adult pairing itself visible again.
  const bandOfAdult = (adultId: string) => BANDS.find((b) => placed[b.id] === adultId);
                  ...
                  {assignedAdults.map((a) => {
                    ...
                    >
                      ...
                      <span className="tweak-body">
                        <b>{a.icon} {a.name}</b>
                        {band && <small>{band.icon} {band.name}を担当</small>}
                      </span>
                    </button>
                    );
                  })}
