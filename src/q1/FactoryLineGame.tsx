...
export default function FactoryLineGame({ onComplete, onPartialComplete }: Q1GameProps) {
  ...
  const lineBoard = (
    <div className="factory-line">
      {steps.map((s) => {
        // 2026-09-13 UX fix: finding the bottleneck needs comparing rate/
        // stop/queue/loss MAGNITUDE across all 6 stations (see
        // factoryLineLogic.ts's s6-also-has-loss comment) -- a single-station
        // toggle (openStep) made that a memorize-one-close-it-open-the-next
        // exercise. Tapping a station now reveals it for good (seenData,
        // already tracked for the "seen enough stations" gate below), so
        // every station opened so far stays visible for side-by-side
        // comparison.
        const revealed = seenData.includes(s.id);
        return (
          <button
            key={s.id}
            className={`fstep ${revealed ? "open" : ""} ${
              picked === s.id ? "picked" : ""
            }`}
            onClick={() => {
              if (phase === "found") {
