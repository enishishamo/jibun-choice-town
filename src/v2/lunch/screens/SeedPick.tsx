...
// TEMP_IMPLEMENTATION_ONLY layout; icons + wording are DESIGN_NEEDED (DN-09).
...
export default function SeedPick({ onDone }: { onDone: (seedId: string) => void }) {
  ...
  return (
    <section className="lsc lsc-seed" aria-label="seed pick">
      ...
      <div className="lsc-seeds">
        {COPY.seed.options.map((o) => (
          <button
            key={o.id}
            type="button"
            className={`lsc-seed-btn ${picked === o.id ? "on" : ""}`}
            onClick={() => setPicked(o.id)}
          >
            <span className="lsc-seed-icon lmp-ph">DN-09</span>
            {o.label}
          </button>
        ))}
      </div>
