...
SINCE r4 (FAIL 80, 0 blockers, 1 HIGH, 2 MEDIUM — all three in the harness, none in the game), all three are repaired and mutation-tested:
- the `MutationObserver` now also watches `class` and `style`. NOTE: your HIGH did not reproduce — a score gated on `.lmp.is-settled::before` (M14) and one gated on the bead's own band class (M17) were BOTH caught by the old aria-only observer, because React never changes a class here without also mutating nodes elsewhere in the same render, and `record()` re-reads generated content for the whole document on any mutation. The filter was widened anyway so the check does not depend on that coincidence. Verify this reasoning; if you still believe there is a reachable case, give the concrete mutation.
- the reduced-motion pass now drives the ENTIRE journey: build, first send, delivery trouble, generic rebuild (try each still-choosable dish, keep the one that brings the school back), second send, and an assertion that the board is gone and the reveal names the job. Mutation M15 (`.lmp-school { pointer-events: none }` under reduced motion) is caught by name.
- the harness now sweeps 320x568, 375x667, 375x812, 390x844 and 430x932, asserting no horizontal scroll, no vertical overflow, exactly nine dishes all fully on screen, no two touch areas intersecting, and no live control under 44x44. The first run of this sweep found a REAL product defect: at 320x568 the tray recesses were 61x43 and the milk recess 52x40. Fixed in `lunchMenuPlay.css`'s `max-height: 620px` block with `min-height: 46px` against a 48px row pitch. Mutation M16 (remove that rule) is caught by name.
Also since r4: six generated pictures replaced clay-SVG fallbacks 1:1 with no geometry change — three WORLD MAP tiles (`map/grow.png`, `map/cook.png`, `map/menu.png`) through `LUNCH_ASSETS.map.spot`, three KNOW THE JOB scenes (`scenes/kitchen.png`, `scenes/classroom.png`, `scenes/talk.png`) through a new optional `scenes` prop on `KnowTheJob`. Two generated pictures were deliberately NOT wired and are open design items: `map/bento.png` (its empty interior is only ~57%x32% of the frame, so the five spots do not fit the portrait map without a layout redesign) and `gauge-rack.png` (its four painted hollows are all ~17% wide while the four GOOD BANDs are ~21/29/38/19%, so a painted hollow would contradict the rule the bead obeys; the hollow stays computed from `bandOnTrack()`). And `lw-road.is-partial` no longer draws `stroke-dasharray: 50 100`, which rendered as one solid bar out of the spot.
Check all of that, and run your own mutation test again.
