#!/usr/bin/env node
// Design-stage exploit simulation for leak-detective / t1-night-listening (v2).
...
// think-again step (a new listen or a record comparison), content-light
...
function simulate(c, plan) {
  ...
  for (let guard = 0; guard < 60; guard++) {
    ...
    if (a.type === "valve") {
      ...
      thinkAgainDone = false; // gate the next report behind a listen or a compare
...
// content-light strategies "compare" for free when gated (they may not be able to listen)
const unlockOr = (v, action) => (v.secondReportUnlocked ? action : { type: "compare" });

const S = {
  // content-blind: 2 distinct random reports
  random_report: (rand) => { const used = new Set(); return (v) => { if (v.budgets.reports <= 0) return { type: "end" }; const p = pickNew(rand, allPoints, used); if (!p) return { type: "end" }; used.add(key(p)); return unlockOr(v, { type: "report", ...p }); }; },
  ...
  listen_random_report_loudest_any: (rand) => { const used = new Set(); return (v) => {
    ...
    return sorted[i] && v.budgets.reports > 0 ? unlockOr(v, { type: "report", seg: sorted[i].seg, point: sorted[i].point }) : { type: "end" }; }; },
  ...
  flow_then_two_distinct_reports: (rand) => { const used = new Set(); return (v) => {
    ...
    return unlockOr(v, { type: "report", ...p }); }; },
  ...
  flow_then_gradient: () => (v) => {
    ...
    if (best && v.budgets.reports > 0) return unlockOr(v, { type: "report", seg, point: best.point });
    return { type: "end" };
  },
};
...
const verdict = {
  ...
  think_again_gate_enforced: gateBlocked === 200,
};
const out = { generated_at: new Date().toISOString(), rules: { segments: SEGS, points_per_segment: P, budgets: B, reading_fields: T.sound_reading.fields }, cases_per_strategy: N, strategies: results, solvability: { solvable, total, first_try: firstTry, worst_listens_used: worstListens, worst_valve_ops_used: worstValves }, think_again_gate: { trials: 200, second_report_refused_without_new_evidence: gateBlocked }, verdict };
