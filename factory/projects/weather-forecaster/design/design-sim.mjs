#!/usr/bin/env node
// Design-stage exploit simulation for weather-forecaster / t1b-storm-night-team.
...
//
// round-3 review fix: the mid-game 解除 action was removed entirely (a
// warning, once issued, stands until morning) because no checkable proxy for
// the real cancellation criterion could be built without either giving the
// player future radar values (an answer leak) or being defeated by a real
// multi-step lull within the same weather system (state_table.json
...
// plan: function(step, view) -> {warn:Set<town>} decided BEFORE advancing
// from `step`. `view` exposes only what the screen shows: current meters,
// thresholds, current rain per town, which towns have a standing warning /
// 避難情報, and past rain (what the child has seen). There is no cancel --
// once issued, a warning stands until morning.
function simulate(pathId, plan) {
  ...
  const warnedAt = {}; // town -> step warning issued (standing, permanent)
  ...
  for (let step = 1; step <= STEPS; step++) {
    ...
    for (const id of TOWNS) {
      if (crossedAt[id] === undefined && meter[id] >= thr[id]) {
        ...
        const safe = evacuatedFrom[id] !== undefined && evacuatedFrom[id] <= step + 1;
        outcome[id] = safe ? "safe" : "late";
      }
    }
    history.push({ step, meter: { ...meter }, standing: { ...warnedAt }, crossed: { ...crossedAt } });
  }
  let falseAlarms = 0;
  for (const id of TOWNS) {
...
const S = {
  ...
  warn_where_raining_now: (step, v) => ({ warn: TOWNS.filter((id) => v.rain[id] > 0 && v.standing[id] === undefined) }),
...
for (const [name, plan] of Object.entries(S)) {
  named[name] = Object.fromEntries(PATHS.map((p) => { const r = simulate(p, plan); return [p, { grade: r.grade, late: r.late, falseAlarms: r.falseAlarms, outcome: r.outcome }]; }));
  named[name].wins_all_paths = PATHS.every((p) => ["perfect", "success"].includes(named[name][p].grade));
}

// ---- exhaustive path-INDEPENDENT fixed schedules: per town, warn at step k in {0=never,1..6} ----
...
const result = {
  ...
  rules: { lead_time_steps: LEAD, steps: STEPS, towns: T.towns, cancel: "none (removed in round-3 repair; a warning stands until morning)" },
  path_facts: facts,
  ...
  verdict: {
    no_content_blind_strategy_wins_all_paths: fixed.wins_all_paths === 0 && !named.never.wins_all_paths && !named.all_step1.wins_all_paths && !named.warn_where_raining_now.wins_all_paths && !named.all_step3.wins_all_paths,
    legitimate_meter_lead_rule_wins_all_paths: named.meter_lead_rule.wins_all_paths,
    every_path_solvable: PATHS.every((p) => !!solvable[p]),
  },
};
writeFileSync(join(HERE, "design-sim-result.json"), JSON.stringify(result, null, 2) + "\n");
console.log(JSON.stringify({ path_facts: facts, named: Object.fromEntries(Object.entries(named).map(([k, v]) => [k, { wins_all_paths: v.wins_all_paths, grades: PATHS.map((p) => v[p].grade) }])), fixed: { total: fixed.total, wins_all_paths: fixed.wins_all_paths, wins_per_path: fixed.wins_per_path, examples: fixed.wins_all_paths_examples }, verdict: result.verdict }, null, 2));
