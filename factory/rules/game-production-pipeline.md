...
4. **運用パラメータが非公開の職業では、時間・順序・閾値に依存しない設計しか
   許されない。** 「はこぶ」は3ラウンドとも『緊張を作っている規則が未確認事実の
   発明である』で落ちた。配送計画の別紙・報告項目・対処手順・記録簿の欄名・
   適量の基準・容量上限は、8文書7自治体を当たって一つも公開されていない。
   ゲームの緊張は普通そこから来るので、その題材では作るたびに発明になる。
   パラメータを要求しない文法（空間的な完全性、実在する文書との突合、
   対称な二値応答）に限る。「何分以内」「何個まで」「どれくらいが適量」を
   要求する設計は、資料の有無を先に確認してから着手する。

5. **見送りの理由は、事実不足とゲーム性不足を必ず分けて書く。**
   RECORDING を見送った際に、欄名が未確認な行と、明文がそろっている行を
   一括して「事実が足りない」と書いた。後者は発明が要らず、本当の理由は
   ゲームとして弱いことだった。曖昧にすると、後から事実を足そうとする人が
   無駄足を踏む。

6. **critic の指摘も、機械で確かめられるものは確かめる。**
