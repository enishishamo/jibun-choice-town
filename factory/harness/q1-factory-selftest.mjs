...
// NN: the same replacement-attack, moved to an EXTRA (non-schema-required) field, is ALSO refused (design review r9 Human Decision: fail-closed by default, not "required fields only")
{
  init("selftest-nn");
  submitAll("selftest-nn");
  const fsBaseline = { ...ART.fact_sheet, candidate_reference_cards: { heart_failure: "original claim text, now to be corrected" } };
  run(PIPE, ["submit", "selftest-nn", "fact_sheet", "--file", fx("nn-fs0.json", fsBaseline)]);
  const ev = fx("nn-ev.json", { claim_removed: "x", cited_source: "https://example.gov/x", source_does_not_support_because: "y", core_ae_translation_impact: "none" });
  // same length, but a DIFFERENT claim substituted into an EXTRA field the schema doesn't require
  const fsReplaced = { ...fsBaseline, candidate_reference_cards: { heart_failure: "a totally different assertion, same length as before" } };
  const fcReplace = run(PIPE, ["fact-correct", "selftest-nn", "fact_sheet", "--file", fx("nn-fs1.json", fsReplaced), "--evidence", ev, "--reason", "trying to swap an extra field's content"]);
  // a genuine narrowing of the SAME extra field must still be accepted
  const fsGenuine = { ...fsBaseline, candidate_reference_cards: { heart_failure: "original claim" } };
  const fcGenuine = run(PIPE, ["fact-correct", "selftest-nn", "fact_sheet", "--file", fx("nn-fs2.json", fsGenuine), "--evidence", ev, "--reason", "genuinely narrowing the extra field"]);
  // a brand-new extra field appearing out of nowhere is refused too
  const fsNewField = { ...fsBaseline, a_brand_new_field_nobody_declared: "surprise content" };
  const fcNewField = run(PIPE, ["fact-correct", "selftest-nn", "fact_sheet", "--file", fx("nn-fs3.json", fsNewField), "--evidence", ev, "--reason", "trying to sneak in an undeclared new field"]);
  record("NN", "a replacement attack moved to an extra (non-required) field is refused, a genuine narrowing of that same extra field is accepted, and an undeclared brand-new field is refused", fcReplace.status === 1 && fcReplace.json?.accepted === false && fcGenuine.status === 0 && fcGenuine.json?.accepted === true && fcNewField.status === 1 && fcNewField.json?.accepted === false, { replace_reason: fcReplace.json?.reason, genuine_accepted: fcGenuine.json?.accepted, new_field_reason: fcNewField.json?.reason });
}

// OO: array narrowing matches items as an order-preserving SUBSEQUENCE, not index-paired -- dropping a MIDDLE item is accepted, reordering/replacing/adding is not (design review r10 Human Decision)
{
  init("selftest-oo");
  submitAll("selftest-oo");
  const fsBaseline = { ...ART.fact_sheet, expertise: ["first item", "middle unsupported claim", "last item"] };
  run(PIPE, ["submit", "selftest-oo", "fact_sheet", "--file", fx("oo-fs0.json", fsBaseline)]);
  const ev = fx("oo-ev.json", { claim_removed: "x", cited_source: "https://example.gov/x", source_does_not_support_because: "y", core_ae_translation_impact: "none" });
  const fsMiddleDropped = { ...ART.fact_sheet, expertise: ["first item", "last item"] }; // dropped the MIDDLE item -- later item now shifts to index 1
  const fcDrop = run(PIPE, ["fact-correct", "selftest-oo", "fact_sheet", "--file", fx("oo-fs1.json", fsMiddleDropped), "--evidence", ev, "--reason", "dropping the middle unsupported item"]);
  const fsReordered = { ...ART.fact_sheet, expertise: ["last item", "first item"] }; // same items, wrong order -- must be refused
  const fcReorder = run(PIPE, ["fact-correct", "selftest-oo", "fact_sheet", "--file", fx("oo-fs2.json", fsReordered), "--evidence", ev, "--reason", "trying to reorder instead of narrow"]);
  record("OO", "dropping a MIDDLE array item is accepted as a narrowing (subsequence match, not index-paired), while reordering the same items is refused", fcDrop.status === 0 && fcDrop.json?.accepted === true && fcReorder.status === 1 && fcReorder.json?.accepted === false, { drop_accepted: fcDrop.json?.accepted, reorder_reason: fcReorder.json?.reason });
}
