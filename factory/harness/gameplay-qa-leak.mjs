...
const S = {
  ...
  flow_then_guess_points: (rand) => { const used = new Set(); return (v) => {
    ...
    if (v.budgets.reports <= 0) return { type: "end" };
    if (v.closed) return { type: "reopen" }; // parity with design-sim flow_then_two_distinct_reports: restore supply before placing the pickup (a silent point cannot be reported)
...
// ---- 4. rules and budgets ----
{
  ...
  check("a report while the segment is closed resolves by the point alone AND the crew restores supply first (closed → null, silent records gone) — hit and miss", (() => { let t = stateFor(c); t = listen(t, "C", 5).state; t = listen(t, "C", 4).state; t = closeValve(t, "C").state; t = listen(t, "C", 2).state; const before = readingAt(publicView(t), "C", 2); const r = report(t, "C", 5); const okHit = r.ok && r.hit === true && r.state.closed === null && readingAt(publicView(r.state), "C", 2) === undefined && readingAt(publicView(r.state), "C", 4)?.level === 4; let u = stateFor(c); u = listen(u, "C", 4).state; u = closeValve(u, "C").state; const m = report(u, "C", 4); const okMiss = m.ok && m.hit === false && m.state.closed === null && m.state.unlocked === false; return before?.continuity === "silent" && okHit && okMiss; })());
  check("the component announces the crew restoring supply when a valve was closed at report time (source check)", (() => { const src = readFileSync(new URL("../../src/q1/LeakTraceGame.tsx", import.meta.url), "utf8"); return src.includes("修理班が弁${v.closed}を開けて水を戻した") && src.includes("修理班が水を戻して掘っている"); })());
  ...
  check("hd-2 item 4: a silent reading is not evidence — reporting a point heard as silent is refused; after reopen + a real listen it is allowed", (() => { let t = stateFor(c); t = closeValve(t, "C").state; t = listen(t, "C", 5).state; if (reportBlocked(t, "C", 5) !== "silent_not_evidence" || report(t, "C", 5).ok) return false; t = openValve(t).state; if (reportBlocked(t, "C", 5) !== "not_heard") return false; t = listen(t, "C", 5).state; return reportBlocked(t, "C", 5) === null && report(t, "C", 5).hit === true; })());
...
// ---- 5. the public view never exposes the hidden truth ----
{
  ...
  // design review r9 MEDIUM: a record heard while open stays valid after its segment is closed, but the LIVE reactions
  // (house-window blink, latest-reading waveform) must stop while that segment is closed
  check("design r9: live blink and the latest-reading waveform are gated on the segment being open (source check)", (() => { const blinkLine = src.split("\n").find((l) => l.includes("const blink = ")); const rowLine = src.split("\n").find((l) => l.includes("{lastReading && lastHeard &&")); return !!blinkLine && blinkLine.includes("v.closed !== seg") && !!rowLine && rowLine.includes("v.closed !== lastHeard.seg"); })());
