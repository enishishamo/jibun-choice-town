# Q1 Factory self-test — 2026-09-23
...
| E | review FAIL → gate refused; codex-task-shaped evidence refused; creator==reviewer never satisfies the gate | FAIL | mechanical |
...
| J | open Human Decision (mascot) blocks gate and submissions; resolving it re-enables the gate | FAIL | mechanical |
| K | GAME_DESIGN_READY → game_spec accepted with provenance to game_translations@1 → SPEC_READY | FAIL | mechanical |
...
| P | implementation review FAIL → release-ready refused | FAIL | mechanical |
| Q | all design+spec+art(none)+impl+QA+reviews PASS and linked task passes can-deploy → RELEASE_CANDIDATE; without a linked task it is refused | FAIL | mechanical |
| R | product_identity_impact=YES without approval blocks release-ready (via task-state can-deploy); an explicit approval record clears it | FAIL | mechanical |
...
| U | scope_core v1→v2 marks ae/play_seeds/…/game_translations STALE + review stale; gate refused; building on a STALE source is refused | FAIL | mechanical |
| V | evidence with observation==interpretation refused; LOW is logged only; HIGH on a RELEASED game returns it to FIRST_PLAY_UX (state REPAIRING) | FAIL | mechanical |
...
| X | game_spec v1→v2 (downstream) keeps the design review non-stale; core_back_check v1→v2 (design stage) stales it | FAIL | mechanical |
